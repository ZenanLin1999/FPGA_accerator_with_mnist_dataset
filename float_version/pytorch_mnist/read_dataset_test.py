...
# 读取标签数据集
with open('./mnist/MNIST/raw/train-labels-idx1-ubyte', 'rb') as lbpath:
    labels_magic, labels_num = struct.unpack('>II', lbpath.read(8))
    labels = np.fromfile(lbpath, dtype=np.uint8)

# 读取图片数据集
with open('./mnist/MNIST/raw/train-images-idx3-ubyte', 'rb') as imgpath:
